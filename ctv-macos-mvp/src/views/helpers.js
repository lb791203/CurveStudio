...
export function renderImportAudit(audit) {
  if (audit.kind === "empty" || (audit.title === "未导入数据" && !audit.rawRowCount && !audit.measurementCount && !audit.usableCount)) {
    return `
      ...
      <div class="empty-state compact-empty">
        ...
        <span>${escapeHtml(t("waiting_measurement_data_help", "请选择测量文件、载入示例，或在手动表录入现场数据。"))}</span>
      </div>
  ...
  return `
    ...
    ${audit.messages.length ? `<p>${escapeHtml(t("hint_label", "提示"))}: ${escapeHtml([...new Set(audit.messages)].join(" / "))}</p>` : `<p>${escapeHtml(t("hint_label", "提示"))}: ${escapeHtml(t("data_structure_ok", "数据结构满足当前计算。"))}</p>`}
  `;
}
